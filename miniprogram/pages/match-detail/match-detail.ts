...
Page({
  data: {
    id: 0,
    match: {} as any,
    signups: [] as SignupRow[],
    displayMembers: [] as any[],
  ...
  async load() {
    ...
    const signups = await request<SignupRow[]>(`/matches/${this.data.id}/signups`);
  ...
  async submitSignup(status: string) {
...
function buildStats(rows: SignupRow[]) {
...
function buildMembers(rows: SignupRow[]) {
  if (!rows.length) {
    return [
